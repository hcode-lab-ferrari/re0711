...
const ComponentPage: NextPage<ComponentPageProps> = ({ amount }) => {
  ...
  const onSubmit: SubmitHandler<FormData> = ({
    ...
    if (
      !cardDocument ||
      cardDocument.length < 11 ||
      cardDocument.length > 14 ||
      (!isCNPJ(cardDocument) && !isCPF(cardDocument))
    ) {
    ...
    mp.createCardToken({
      ...
      securityCode: cvv,
      identificationType:
        cardDocument.match(/\d/g)?.join('').length === 11 ? 'CPF' : 'CNPJ',
      identificationNumber: cardDocument,
    })
      .then(
        ({
          first_six_digits: cardFirstSixDigits,
          last_four_digits: cardLastFourDigits,
          id: cardToken,
        }: {
          id: string;
  ...
  const createPayment = (data: ScheduleCreate) => {
    axios
      .post('/api/schedules/payment', data)
  ...
  const initMercadoPago = () => {
    setMp(
      new MercadoPago(process.env.MERCADO_PAGO_PUBLIC_KEY, {
        locale: 'pt-BR',
      })
    );
  };
    ...
    if (mp && bin.length === 6) {
      mp.getInstallments({
        ...
        if (result.length) {
          ...
          setPaymentMethodId(option.payment_method_id);
          setPaymentTypeId(option.payment_type_id);

          setValue('installments', '1');

          setInstallmentOptions(
            option.payer_costs.map((cost: any) => {
              return {
  ...
  useEffect(
    () => clearErrors(),
    [number, expiry, cvv, name, installments, cardDocument]
  );

  useEffect(() => {
    if (mp && bin.length === 6 && paymentMethodId) {
      mp.getIssuers({
        bin,
        paymentMethodId,
      }).then((result: any) => {
        setIssuers(
          result.map(({ id, name }: any) => ({
            id,
            name,
          }))
        );
            ...
            <div className="form-fields">
              ...
              {issuers.length > 1 && (
                <div className="field">
                  <select
                    id="issuers"
                    {...register('bank', {
                      required: 'Seelcione o banco emissor.',
                    })}
                  >
                    {issuers.map(({ id, name }) => (
              ...
              <div className="field">
                <select
                  ...
                  {...register('installments', {
                    required: 'Seelcione a quantidade parcelas.',
                  })}
                >
                  {installmentOptions.map((option) => (
                    <option
                      key={`installment-${option.number}`}
                      value={option.number}
                    >
                      {option.description}
                    </option>
              ...
              <div className="field">
                <IMaskInput
                  ...
                  value={cardDocument}
                />
                <label htmlFor="number">CPF ou CNPJ do Títular do Cartão</label>
              </div>
            </div>

            <div className="form-card">
              <div
                className={['faces', cardBack ? 'flipped' : ''].join(' ')}
                id="credit-card"
              >
                <svg
                  className="front"
                ...
                >
                  <g transform="translate(-4292.083 3513.639)">
                    <g transform="translate(4292.083 -3513.639)">
                      ...
                      >
                        <tspan x="-39" y="0" className="expiry">
                          {expiry}
                        </tspan>
                      </text>
                      <text
                    ...
                    <text
                      ...
                      fontFamily="Teko-Regular, Teko"
                    >
                      <tspan x="-27.3" y="0" className="number-1">
                    ...
                    <text
                      ...
                      fontFamily="Teko-Regular, Teko"
                    >
                      <tspan x="-27.3" y="0" className="number-2">
                    ...
                    <text
                      ...
                      fontFamily="Teko-Regular, Teko"
                    >
                      <tspan x="-27.3" y="0" className="number-3">
                    ...
                    <text
                      ...
                      fontFamily="Teko-Regular, Teko"
                    >
                      <tspan x="-27.3" y="0" className="number-4">
                ...
                >
                  ...
                  <g transform="translate(-4292.083 3734.497)">
                    ...
                    <text
                      ...
                      fontFamily="Teko-Regular, Teko"
                    >
                      <tspan x="-15.795" y="0" className="cvv">
                        {cvv}
                      </tspan>
                    </text>
                    <text
                    ...
                    >
                      <tspan x="89.566" y="21" className="name">
                        {name}
                      </tspan>
                    </text>
                  </g>
...
export const getServerSideProps = withAuthentication(async (context) => {
  try {
    const { data } = await axios.get<PaymentResponse>('/payment', {
      ...
      params: {
        services: context.req.session.schedule.services?.toString(),
      },
    });
